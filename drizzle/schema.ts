...
export const imageGenerations = mysqlTable(
  ...
  table => [
    index("image_generations_owner_created_idx").on(
      table.ownerOpenId,
      table.createdAt
    ),
  ]
...
export const automationWorkflows = mysqlTable(
  "automationWorkflows",
  {
    id: int("id").autoincrement().primaryKey(),
    ownerOpenId: varchar("ownerOpenId", { length: 64 }).notNull(),
...
export const workflowRuns = mysqlTable(
  ...
  table => [
    index("workflow_runs_workflow_started_idx").on(
      table.workflowId,
      table.startedAt
    ),
  ]
...
export const activityEvents = mysqlTable(
  ...
  table => [
    index("activity_events_owner_created_idx").on(
      table.ownerOpenId,
      table.createdAt
    ),
  ]
