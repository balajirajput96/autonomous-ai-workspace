...
export function StatusPill({ status }: { status: "started" | "succeeded" | "failed" | "info" | "running" }) {
  const styles = {
  ...
  return <span className={cn("inline-flex items-center rounded-full border px-2.5 py-1 text-[11px] font-medium capitalize", styles[status])}>{status}</span>;
}
