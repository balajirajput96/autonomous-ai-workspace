...
export default function DashboardLayout({
  ...
  if (!user.isOwner) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#080914] px-6">
        <div className="max-w-md text-center">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-400">
            This workspace is limited to its owner. No workspace data has been
            shown.
          </p>
          <Button
            variant="outline"
  ...
  return (
    ...
    >
      <LayoutContent setSidebarWidth={setSidebarWidth}>
        {children}
      </LayoutContent>
    </SidebarProvider>
  );
    ...
    <div className="flex min-h-screen items-center justify-center bg-[#080914] px-6">
      <div className="max-w-md text-center">
        <div className="mx-auto flex size-14 items-center justify-center rounded-2xl bg-violet-400/10">
          <LockKeyhole className="size-6 text-violet-300" />
        </div>
        <h1 className="mt-5 text-2xl font-semibold text-white">
          Private workspace
        </h1>
        <p className="mt-2 text-sm leading-6 text-slate-400">
          Sign in with Manus OAuth to enter this owner-only AI workspace.
        </p>
        <Button
          onClick={() => startLogin()}
  ...
  return (
    <>
      <div ref={sidebarRef} className="relative">
        <Sidebar
          collapsible="icon"
          className="border-r border-white/10 bg-[#0d0f20]"
        ...
        >
          <SidebarHeader className="h-[72px] border-b border-white/10 px-3">
            <div className="flex h-full items-center gap-3">
              <button
                ...
                className="flex size-9 shrink-0 items-center justify-center rounded-xl bg-white/5 text-slate-400 transition hover:bg-white/10 hover:text-white"
              >
              ...
              {menuItems.map(item => (
                <SidebarMenuItem key={item.path}>
                  <SidebarMenuButton
                    ...
                    tooltip={item.label}
                    className="h-10 rounded-xl text-slate-400 transition hover:bg-white/5 hover:text-white data-[active=true]:bg-violet-400/15 data-[active=true]:text-white"
                  >
          ...
          <SidebarFooter className="border-t border-white/10 p-3">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className="flex w-full items-center gap-3 rounded-xl px-1 py-1 text-left transition hover:bg-white/5">
                  <Avatar className="size-8 border border-white/10">
                    <AvatarFallback className="bg-violet-400/10 text-xs text-violet-200">
              ...
              <DropdownMenuContent
                align="end"
                className="border-white/10 bg-[#15172a] text-slate-200"
              >
                <DropdownMenuItem
                  onClick={logout}
        ...
        <div
          ...
          className={`absolute right-0 top-0 h-full w-1 cursor-col-resize hover:bg-violet-400/30 ${isCollapsed ? "hidden" : ""}`}
        />
      </div>
      <SidebarInset className="bg-[#080914]">
        {isMobile && (
          <header className="sticky top-0 z-30 flex h-14 items-center gap-2 border-b border-white/10 bg-[#080914]/90 px-3 backdrop-blur">
            <SidebarTrigger className="text-slate-300" />
            <span className="text-sm font-medium text-white">
              {active?.label ?? "AI Workspace"}
            </span>
          </header>
        )}
