...
function textFromLLM(content: string | Array<{ type: "text"; text: string } | unknown>) {
...
export async function executeWorkflow(workflowId: number) {
  ...
  await db.addActivity(workflowRow.ownerOpenId, "workflow", `Started scheduled workflow: ${workflowRow.name}`, "started");
  try {
    const response = await invokeLLM({
      messages: [
        { role: "system", content: "You execute a private workspace automation. Produce a concise, useful result based only on the requested action. Do not claim you contacted external systems or completed actions you cannot verify." },
        { role: "user", content: `Workflow name: ${workflowRow.name}\nTrigger: ${workflowRow.trigger}\nRequested action: ${workflowRow.action}` },
      ],
      maxTokens: 900,
    });
    const output = textFromLLM(response.choices[0]?.message.content ?? "");
    await db.finishWorkflowRun(runId, "succeeded", { output });
    await db.addActivity(workflowRow.ownerOpenId, "workflow", `Completed scheduled workflow: ${workflowRow.name}`, "succeeded");
    await notifyOwner({ title: "Automation completed", content: `“${workflowRow.name}” completed successfully.` });
    return { output };
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown workflow error";
    await db.finishWorkflowRun(runId, "failed", { error: message });
    await db.addActivity(workflowRow.ownerOpenId, "workflow", `Failed scheduled workflow: ${workflowRow.name}`, "failed");
    await notifyOwner({ title: "Automation failed", content: `“${workflowRow.name}” failed: ${message.slice(0, 240)}` });
    throw error;
  }
}
